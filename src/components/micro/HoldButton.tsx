// Vendored from reactbits.dev's "Hold Button" (MIT + Commons Clause,
// https://github.com/DavidHDev/react-bits), converted from JS+CSS to TS;
// behavior unmodified. No third-party deps — pure React + native APIs.
import {
  useEffect,
  useId,
  useLayoutEffect,
...
export default function HoldButton({
  ...
  const motionRef = useRef({ raf: 0, p: 0, from: 0, to: 0, start: 0 })
  ...
  const handlePointerDown = (e: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      // Pointer capture can fail on some platforms — release still works via pointerup/leave.
    }
  }

  const endPointer = (e: ReactPointerEvent<HTMLButtonElement>, options?: { drifted?: boolean }) => {
