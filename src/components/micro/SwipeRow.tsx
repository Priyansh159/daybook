// Vendored from reactbits.dev's "Swipe Row" (MIT + Commons Clause,
// https://github.com/DavidHDev/react-bits), converted from JS+CSS to TS;
// behavior unmodified.
import {
  useEffect,
  useId,
...
export default function SwipeRow({
  ...
  const move = (e: PointerEvent) => {
    ...
    if (g.grab === null) {
      ...
      // Capture is deferred until a real drag starts (past the hysteresis
      // threshold) so a plain tap/click on row content still fires its own
      // click event instead of being retargeted to this surface.
      try {
        surface.current?.setPointerCapture(g.id)
      } catch {
        // Ignore — dragging still works without capture on some platforms.
      }
    }
  ...
  const act = (a: SwipeAction, e: ReactMouseEvent) => {
  ...
  const onToggleClick = (e: ReactMouseEvent<HTMLButtonElement>) => {
    if (disabled || phase !== 'idle' || n === 0) return
    e.stopPropagation()
    if (open) closeNow()
    else openNow()
      ...
      <div className="swipe-row__clip">
        ...
        >
          ...
          {primary ? (
            <motion.div className="swipe-row__block" style={{ transform: blockXf }}>
              <motion.button type="button" className="swipe-row__action swipe-row__action--commit" style={{ transform: glyphXf }} onClick={(e) => act(primary, e)}>
                <span className="swipe-row__glyph">
                  <span className="swipe-row__icon">{primary.icon ?? <HugeiconsIcon icon={Delete02Icon} size={20} strokeWidth={2} />}</span>
        ...
        <motion.div ref={surface} className="swipe-row__surface" style={{ transform: surfaceXf }} onPointerDown={down}>
          ...
          <button
            ...
            aria-label={open ? 'Close actions' : `${n} ${n === 1 ? 'action' : 'actions'}`}
