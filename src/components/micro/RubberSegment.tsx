// Vendored from reactbits.dev's "Rubber Segment" (MIT + Commons Clause,
// https://github.com/DavidHDev/react-bits), TS + Tailwind variant, unmodified.
import {
  useEffect,
  useLayoutEffect,
  useRef,
...
export default function RubberSegment({
  ...
  const land = (to: number, v: number | null, flick: boolean, withSquash: boolean) => {
    ...
    const velocityFor = (mv: MotionValue<number>) => clamp(v === null ? mv.getVelocity() : v, -MAX_VELOCITY, MAX_VELOCITY)
    animate(lead, leadTo, {
  ...
  const handlePointerDown = (e: ReactPointerEvent<HTMLButtonElement>, i: number) => {
    if (disabled || drag.current || e.button !== 0) return
    box.current = trackRef.current?.getBoundingClientRect() ?? null
    try {
      e.currentTarget.setPointerCapture(e.pointerId)
    } catch {
      // Pointer capture can fail on some platforms — dragging still works via move/up handlers.
    }
    const x = localX(e)
  ...
  return (
    <div
      ...
      className={`group relative inline-grid grid-flow-col auto-cols-auto select-none touch-pan-y align-middle [-webkit-tap-highlight-color:transparent] [-webkit-touch-callout:none] [font-family:inherit] rounded-[var(--rs-radius)] p-[var(--rs-inset)] [background:var(--rs-track)] data-[equal]:auto-cols-[minmax(0,1fr)] data-[held]:cursor-grabbing aria-disabled:pointer-events-none aria-disabled:opacity-50${className ? ` ${className}` : ''}`}
      style={
        {
          '--rs-track': trackColor,
          '--rs-thumb': thumbColor,
    ...
    >
      {list.map((item, i) => (
        <button
          ...
          disabled={disabled}
          className="group-data-[draggable]:aria-checked:cursor-grab group-data-[held]:cursor-grabbing m-0 inline-flex h-[calc(var(--rs-h)-var(--rs-inset)*2)] min-w-[var(--rs-min)] cursor-pointer items-center justify-center gap-1.5 rounded-[var(--rs-thumb-radius)] border-0 bg-transparent px-[var(--rs-pad)] py-0 text-[length:var(--rs-font)] font-medium leading-none whitespace-nowrap [color:var(--rs-ink)] opacity-70 outline-none [font:inherit] [transition:opacity_160ms_ease,transform_160ms_var(--rs-ease-out)] motion-reduce:[transition:opacity_160ms_ease] aria-checked:cursor-default focus-visible:[outline-color:var(--rs-thumb)] focus-visible:outline-2 focus-visible:outline-offset-[3px] data-[pressed]:[transform:scale(0.96)] [@media(hover:hover)_and_(pointer:fine)]:[&[aria-checked=false]:hover]:opacity-90"
      ...
      <motion.div
        className="group-data-[equal]:auto-cols-[minmax(0,1fr)] pointer-events-none absolute inset-[var(--rs-inset)] grid grid-flow-col auto-cols-auto [background:var(--rs-thumb)] [color:var(--rs-ink-active)]"
        aria-hidden="true"
        style={{ clipPath }}
      >
        {list.map((item) => (
          <span
            key={item.value}
            className="m-0 inline-flex h-[calc(var(--rs-h)-var(--rs-inset)*2)] min-w-[var(--rs-min)] cursor-default items-center justify-center gap-1.5 rounded-[var(--rs-thumb-radius)] border-0 bg-transparent px-[var(--rs-pad)] py-0 text-[length:var(--rs-font)] font-medium leading-none whitespace-nowrap [color:inherit] outline-none [font:inherit] [transition:opacity_160ms_ease,transform_160ms_var(--rs-ease-out)] motion-reduce:[transition:opacity_160ms_ease]"
          >
            {item.icon}
            {item.label}
          </span>
