// Adapted from reactbits.dev's "Prismatic Burst" background (MIT + Commons
// Clause, https://github.com/DavidHDev/react-bits) — a lightweight WebGL
// shader background built on `ogl`, not three.js. Used behind the login card.
...
export function PrismaticBurst({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- one-time WebGL scene setup; props are pushed via the uniforms effect below.
  }, [])

  useEffect(() => {
    const canvas = rendererRef.current?.gl?.canvas
    if (canvas) {
      canvas.style.mixBlendMode = lightMode ? 'normal' : mixBlendMode && mixBlendMode !== 'none' ? mixBlendMode : ''
    }
  ...
  return <div className={className ?? 'h-full w-full'} ref={containerRef} />
}
