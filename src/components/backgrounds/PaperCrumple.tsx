// @ts-nocheck
// Vendored verbatim from reactbits.dev's "Paper Crumple" (MIT + Commons
// Clause, https://github.com/DavidHDev/react-bits/tree/main/src/ts-tailwind/Micro/PaperCrumple).
// This is a dense cloth/paper-fold physics simulation (typed-array mesh math)
// that isn't hand-modified — type-checking is disabled for this file alone
// rather than risk introducing a bug while satisfying noUncheckedIndexedAccess
// across hundreds of raw array indices. Only used on the 404 page.
...
const PaperCrumple = ({
  ...
  useEffect(() => {
    ...
    function wake() {
      if (!frame && !disposed && ready && inView && !document.hidden && !contextLost) frame = requestAnimationFrame(render)
    }
    ...
    function resize() {
      ...
      scale = paperWidth * Math.min(1, Math.max(1, viewportWidth - 48) / paperWidth, Math.max(1, viewportHeight - 48) / paperHeight)
    ...
    function finish(instant = false) {
      ...
      if (opts.releaseBehavior === 'stay') {
        ...
        amount.target = 0
        memory.target = opts.releaseBehavior === 'creased' ? Math.max(memory.value, peak * clamp(finite(opts.creaseStrength, 0.18), 0, 1)) : 0
      }
      ...
      if (opts.returnToOrigin && opts.releaseBehavior !== 'stay') {
        ...
        const halfWidth = ((Math.abs(Math.cos(baseRotation)) * spanX + Math.abs(Math.sin(baseRotation)) * spanY) * scale) / 2
        const halfHeight = ((Math.abs(Math.sin(baseRotation)) * spanX + Math.abs(Math.cos(baseRotation)) * spanY) * scale) / 2
        const limitX = Math.min(Math.max(0, finite(opts.dragRadius, 180)), Math.max(0, viewportWidth / 2 - halfWidth - 16))
        const limitY = Math.min(Math.max(0, finite(opts.dragRadius, 180)), Math.max(0, viewportHeight / 2 - halfHeight - 16))
      ...
      keyboard = false
      publish(opts.releaseBehavior === 'stay' && amount.target > 0.001 ? 'crumpled' : memory.target > 0.001 ? 'creased' : 'flat')
      wake()
    }
    ...
    function pointerMove(event: PointerEvent) {
      ...
      const maxTilt = reduceMotion ? 0 : THREE.MathUtils.degToRad(clamp(finite(options.current.dragRotation, 10), 0, 60))
    ...
    function keyDown(event: KeyboardEvent) {
      ...
      if (event.key === ' ' || event.key === 'Enter') {
        ...
        peak = Math.max(peak, amount.value)
      } else if (held && keyboard && options.current.draggable && ['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(event.key)) {
        event.preventDefault()
  ...
  }, [src, backSrc, width, height, imageFit, foldCount, foldSharpness, wrinkleDepth, paperColor, roughness, paperTexture, lightIntensity, lightAngle, shadow, shadowOpacity, rotation, seed, detail])
  ...
  const shadowFilter = shadow ? `drop-shadow(0 6px 10px rgb(0 0 0 / ${clamp(finite(shadowOpacity, 0.08), 0, 1)}))` : undefined

  return (
    <div ref={rootRef} className={`relative isolate w-full overflow-hidden ${className}`} style={{ height: sceneHeight, ...style }} data-status={status}>
      <canvas
        ref={canvasRef}
      ...
      {status !== 'ready' && (
        <img
          ...
          alt={alt}
          draggable={false}
          style={{ objectFit: imageFit, transform: `translate(-50%, -50%) rotate(${rotation}deg)`, filter: shadowFilter }}
        />
      )}
      <button
        ref={hitRef}
      ...
      <span className="sr-only" role="status">
        {status === 'loading' ? 'Loading interactive paper.' : status === 'error' ? 'Interactive paper unavailable. Showing the original image.' : ''}
      </span>
    </div>
  )
}
